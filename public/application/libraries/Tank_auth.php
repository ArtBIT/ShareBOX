...
class Tank_auth
{
    ...
    public function get_current_user()
    {
        static $user = null;
        if (is_null($user)) {
            if ($row = $this->ci->users->find_row(array('id' => $_SESSION['user_id']))) {
                $user = $row;
            } else {
                $user = 0;
            }
     ...
     * @return  array
     */
    public function create_user($username, $email, $password, $firstname, $lastname, $email_activation)
    {
        if ((strlen($username) > 0) and !$this->ci->users->is_username_available($username)) {
            ...
            $data = array(
                ...
                'digest'    => $this->ci->users->generate_digest_hash($username, $password),
                'email'     => $email,
            ...
            if (!is_null($res = $this->ci->users->create_user($data, !$email_activation))) {
                ...
                $this->ci->Grupe_model->add_user_to_default_group($data['user_id']);
    ...
    public function reset_password($user_id, $new_pass_key, $new_password)
    {
        if ((strlen($user_id) > 0) and (strlen($new_pass_key) > 0) and (strlen($new_password) > 0)) {
            if (!is_null($user = $this->ci->users->get_user_by_id($user_id, true))) {
                ...
                $this->ci->users->reset_digest_auth($user->username, $new_password);
    ...
    public function change_password($old_pass, $new_pass)
    {
        ...
        if (!is_null($user = $this->ci->users->get_user_by_id($user_id, true))) {
            ...
            if ($hasher->CheckPassword($old_pass, $user->password)) {           // success
                ...
                $this->ci->users->reset_digest_auth($user->username, $new_pass);
    ...
    public function force_login_by_username($username)
    {
        if ($user = $this->ci->users->get_user_by_username($username)) {
            $this->login_user($user);
            return true;
        }
        return false;
    ...
    public function force_login_by_id($id)
    {
        if ($user = $this->ci->users->get_user_by_id($id, true)) {
            $this->login_user($user);
            return true;
        }
        return false;
